...
if (!$conn) {
  ...
  echo "<h5 class='text-center'>"."<p>Aktualny pracownik do edycji</p>"."</h5>";
      $danerazbazy=$conn->query("SELECT * FROM uzytkownicy WHERE id_uzytkownika=$id");
      if($danerazbazy->num_rows>0)
      {
        ...
        {
          $danekzbazy=$conn->query("SELECT * FROM uzytkownicy WHERE id_uzytkownika='$id'");
...
if(isset($_POST["submit"]))
{
  ...
  if(empty($noweimie))
  {
    ...
    $odp=$conn->query("UPDATE uzytkownicy set imie='$noweimie' WHERE id_uzytkownika='$id'");
    $wszystkook=true;
  }
  if(empty($nowenazwisko))
  {
    ...
    $odp=$conn->query("UPDATE uzytkownicy set nazwisko='$nowenazwisko' WHERE id_uzytkownika='$id'");
    $wszystkook=true;
  }
  if(empty($nowytelefon))
  {
    ...
    $odp=$conn->query("UPDATE uzytkownicy set numer='$nowytelefon' WHERE id_uzytkownika='$id'");
    $wszystkook=true;
  }
  if(empty($nowyemail))
  {
    ...
    $odp=$conn->query("UPDATE uzytkownicy set email='$nowyemail' WHERE id_uzytkownika='$id'");
    $wszystkook=true;
  }
